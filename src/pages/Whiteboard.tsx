...
function generateCanvas(freedraw: FreedrawElement) {
...
function renderElement(element: CommonElement, context: CanvasRenderingContext2D) {
  switch (element.type) {
    case 'freedraw':
      ...
      const elementCanvas = oldCache ?? elementCanvasCaches.get(freedraw)!;
...
function renderBoard(canvas: HTMLCanvasElement, { elements }: BoardState, renderConfig: RenderConfig) {
  ...
  const { gridDisplay } = renderConfig;
